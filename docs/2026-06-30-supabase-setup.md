...
- `onecard_events`: 카드 내기·뽑기·주사위 같은 공개 이벤트
- 10개 앱 RPC: 방 생성/참가, 준비 토글, 주사위, 카드 내기/뽑기, 재대결, 상태 조회, 연결 유지, 나가기
- RLS: 자기 방만 읽게 제한
...
7. 선공/후공 이펙트 뒤 카드 내기, 7 무늬 변경, 공격 누적, 뽑기 공개를 확인합니다.
8. 한 판이 끝나면 양쪽 모두 `재대결 신청`을 눌러 같은 방 코드로 새 덱과 주사위 단계가 시작되는지 확인합니다.

기존에 이 SQL을 실행한 프로젝트도 기능 업데이트 후에는 파일 전체를 다시 실행해야 합니다. `create or replace function` 방식이라 기존 테이블과 데이터를 삭제하지 않고 RPC만 갱신합니다.
...
- 브라우저 새로고침은 같은 익명 세션이면 다시 접속할 수 있습니다. 현재 버전은 새로고침 후 방을 자동 복원하지 않으므로 방 코드를 다시 입력하는 흐름은 다음 개선 후보입니다.
