...
- 앱 RPC: 방 생성/참가, 준비 토글, 주사위, 카드 내기/뽑기, 재대결, 상태·기록 조회, 스티커, 점수 공유, 연결 유지, 나가기
- RLS: 자기 방만 읽게 제한
...
기존에 이 SQL을 실행한 프로젝트도 기능 업데이트 후에는 파일 전체를 다시 실행해야 합니다. `host_rating`, `guest_rating`, `round_no` 컬럼은 `add column if not exists`로 추가되고 함수는 `create or replace function`으로 갱신되므로 기존 방 전적·데이터를 삭제하지 않습니다. `round_no`는 같은 방의 각 경기를 고유하게 구분해 새 상대와 경기할 때도 개인 승수·판수가 빠지지 않게 합니다.
