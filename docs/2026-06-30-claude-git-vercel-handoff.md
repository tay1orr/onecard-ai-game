...
- 규칙·레이아웃·AI 반응·멀티 보안·DOM 계약 자동 테스트 51개 통과
- 데스크톱 1280×720, 모바일 390×844 브라우저 점검 완료
- 실제 카드 선택, 7 무늬 변경, AI 응답, 턴 연속 진행 확인
- 5장·16장 손패 배치, 카드 기록, 장난감 상호작용 확인
...
- AI·멀티 공통 셔플과 10장 교차 배분 연출 구현
- 같은 방 재대결 동안 HOST·GUEST 누적 승수 표시
- 진행 중인 방 자동 복원, 이번 판 카드 기록, 6종 오리지널 스티커 구현
- AI 대전 플레이어 스티커와 상황·난이도별 AI 반응 구현
- Supabase Project URL과 Publishable key 입력 완료, 최신 SQL 재실행 필요
- Supabase 익명 사용자 2명 서버 통합 테스트 통과: 준비·취소, 22액션 게임 종료, 재대결 신청·취소와 같은 코드 새 게임 확인
...
- `src/2026-06-30-deal-animation.js`: 셔플·양쪽 5장 배분 연출
...
- `src/2026-07-01-card-art.js`: 공용 광대 조커 카드 그림
...
- 5장씩 배분하는 표준 52장 + 조커 2장 덱
- 같은 무늬 또는 같은 숫자를 낼 수 있음
- 2는 +2, A는 +3이며 2·A·조커로 누적 방어 가능
- 조커는 +5이고 조커로만 방어 가능하며, 공격 성공 후 조커를 낸 플레이어가 원하는 카드 한 장을 낼 수 있음
- J는 턴 건너뛰기
- Q는 2인 대전에서 추가 효과 없는 일반 카드
- K는 2인전에서 추가 효과 없는 일반 카드이며, 3인 이상 확장 시 두 칸 건너뛰기 예정
